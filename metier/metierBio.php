...
Flight::map('validationNom', function(string $table,string $colone,string $value,PDO $con){
    $res = false;
    try{
...
Flight::map('deleteProtocoleChoisi', function($societe,$idCategorieProtocole,$data,PDO $con){
 ...
 Flight::map('getOutProtocoleBySociete', function($idSociete,PDO $con){
     try{
        ...
        $afterWhere = sprintf($afterWhere,$idSociete);
