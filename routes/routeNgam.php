  ...
  if ($_SERVER['REQUEST_METHOD'] == 'OPTIONS') {
    ...
    if (!isset($req->query['id']) || $req->query['id'] == "") {
      ...
      try {
        ...
        if(count($verif)!=0){
          Flight::json(
            new ApiResponse("error", Constante::$ERROR_CODE['400'], $verif, "You have to remove this protocole from all those societes's protocole choisis if you want to delete it"),
            Constante::$ERROR_CODE['400']
          );
        }else{
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'OPTIONS') {
    ...
    if (!isset($req->query['id']) || $req->query['id'] == "") {
      ...
      try {
        ...
        //verification hoe mbola misy societe manana an iny categorie iny ve ka societe valide mbola tsy nfafana
        $verif= GenericDb::find(Societe::class, "societe",
        array(),
        " where idcategoriesociete='".$req->query['id']."' and idcategoriesociete not in(select idCategorieSociete from categorieSocieteDelete) and id not in (select idSociete from societedelete) ",Flight::db());
        if(count($verif)!=0){
          Flight::json(
        ...
        if ($ex->getCode() == 400) {
          ...
          Flight::json(
            new ApiResponse("error", Constante::$ERROR_CODE['500'], null, "server error during delete"),
            Constante::$ERROR_CODE['500']
          );
        }
      }
